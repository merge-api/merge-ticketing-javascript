...
> PaginatedAccountList accountsList(x_account_token, opts)
...
let apiInstance = new MergeTicketingApi.AccountsApi();
...
apiInstance.accountsList(x_account_token, opts, (error, data, response) => {
  if (error) {
    console.error(error);
  } else {
...
> Account accountsRetrieve(x_account_token, id, opts)
...
apiInstance.accountsRetrieve(x_account_token, id, opts, (error, data, response) => {
  if (error) {
    console.error(error);
  } else {
    console.log('API called successfully. Returned data: ' + data);
